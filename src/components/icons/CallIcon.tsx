...
const CallIcon: FC<{ className?: string; color?: string }> = ({ className, color }) => {
    return (
        <div className="relative flex justify-center items-center">
            <svg fill={color || 'currentColor'} className={`h-6= w-6 object-contain absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ${className || ''}`} role="presentation"
